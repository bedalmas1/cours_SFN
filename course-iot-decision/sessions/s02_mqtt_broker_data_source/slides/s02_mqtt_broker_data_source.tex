...
\texttt{présent}, puis \texttt{4}, puis une enveloppe contenant topic, received\_at, retained et payload.
\end{block}
\end{frame}
...
\small
\begin{enumerate}
\item Quel filtre exact définit votre champ de vision ?
\item Combien de topics avez-vous effectivement observés ?
\item Quels champs viennent de l'enveloppe ? Du payload ?
\item Quelle horloge décrit la mesure ? La réception ?
\item Quelle phrase pouvez-vous défendre sans référentiel attendu ?
\end{enumerate}
\textbf{Trace :} une ligne `` j'observe / je peux conclure '' par message.
\end{frame}
...
\end{verbatim}
Les deux comptages doivent afficher \texttt{4}.
\end{frame}

\begin{frame}{TP 2 -- Les cinq réponses à remettre}
\small
\begin{enumerate}
\item Quelles quatre zones sont observées ?
\item Quelle colonne permet de retrouver le périmètre interrogé ?
\item Pourquoi conserver message\_id et les deux horloges ?
\item Que perdez-vous en regroupant immédiatement par capteur ?
\item L'inventaire prouve-t-il le fonctionnement actuel ?
\end{enumerate}
\textbf{Fin du TP :} CSV contrôlé + mini-décision en six champs.
\end{frame}
...
\end{verbatim}
Attendu : 4 observés, 5 attendus, \texttt{complete=False}, confiance faible et topic optronique absent.
\begin{alertblock}{Formulation exacte}
...
\small
\begin{enumerate}
\item Que signifie exactement 4/5 ?
\item Pourquoi 80 \% ne mesure-t-il pas le risque couvert ?
\item Quelle zone a une couverture nulle ?
\item L'absence prouve-t-elle panne ou température normale ?
\item Quel test départage filtre erroné et absence broker ?
\item Quel contrôle renseigne l'état physique réel ?
\end{enumerate}
\end{frame}
...
\begin{enumerate}
\item observation compatible ;
\item vérification discriminante ;
\item résultat qui renforce l'hypothèse ;
\item résultat qui l'affaiblit ;
\item action provisoire et réversible.
\end{enumerate}
\decisionquestion{Quelle vérification réduit l'incertitude le plus vite sans transformer le silence en mesure ?}
\end{frame}
...
\begin{enumerate}
\item \textbf{5 min :} rédiger avec action, périmètre, confiance, preuves, incertitudes et vérification ;
\item \textbf{4 min :} répondre aux cinq questions du décideur critique ;
\item \textbf{6 min :} réviser, citer les fichiers, voter et expliquer l'écart.
\end{enumerate}
...
\small
\begin{enumerate}
\item Votre recommandation vaut-elle pour quatre zones ou cinq ?
\item Dans quel fichier retrouve-t-on chaque preuve ?
\item Quelle hypothèse pourrait renverser la décision ?
\item Que faites-vous pour l'optronique avant 10 h ?
\item L'action reste-t-elle réversible si le diagnostic change ?
\end{enumerate}
\end{frame}
